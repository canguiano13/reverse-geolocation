"""
School sampling via geographic grid.

Splits NY State into a 9x9 grid and samples up to N schools per cell
from the full statewide school list. This gives ~250 geographically
diverse schools covering all of NY without redundancy in dense areas.

Input:  data/inputs/gigamaps_schools_ny.csv  (13,143 NY schools)
Output: data/inputs/schools_selected.csv

Note: the current schools_selected.csv was generated by the peer team
who also manually added school website URLs. Only rerun this if you need
a fresh sample — the existing file is the one used in the paper.
"""
...
def sample_schools(schools, n=N_SAMPLE):
    """Sample up to n schools distributed evenly across a 9x9 NY grid."""
    ...
    sampled = sorted(sampled[:n], key=lambda s: (float(s["latitude"]), float(s["longitude"])))
    return sampled
...
def run(input_file=INPUT_FILE, output_file=OUTPUT_FILE, seed=None):
    ...
    # Drop schools with unknown names or no known education level
    schools = [
